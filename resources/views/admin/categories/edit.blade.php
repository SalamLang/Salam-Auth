...
<div class="container mt-4" xmlns="http://www.w3.org/1999/html">
    <div class="w-100 bg-white p-3 rounded-3 shadow">
        <form action="{{ route("categories.update",$category?->id) }}" method="post" class="w-full h-full">
            @csrf
            @method("PUT")
            <div class="mt-0 flex justify-center items-center">
                <h1 class="text-center">edit category {{$category?->id}}</h1>
            </div>
            <div class="mt-3">
            ...
            <div class="mt-3">
                ...
                <select name="parent_id" id="parent_id" class="form-control mt-1">
                    @if($category?->parent != null)
                        <option value="{{$category?->parent_id}}">{{$category?->parent->title}}</option>
                    @endif
                </select>
                @error('parent_id')
                    <span class="invalid-feedback  text-red-500 d-block" role="alert">
